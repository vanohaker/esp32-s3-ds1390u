...
uint8_t bright_val = 10;
uint8_t bright_temp_val = 10; // Временное значение для формы Brightness
bool bright_bar_s = false;
...
uint8_t mui_brightness_bar(mui_t *mui, uint8_t msg) {
    ...
    uint8_t bar_width = 60; // ширина бара
    ...
    switch(msg) {
        case MUIF_MSG_DRAW:
            if (bright_bar_s) {
                u8g2_DrawFrame(ui, x - 2, y - 2, bar_width + 4, bar_height + 4);
            }
...
muif_t menu_list[] = {
    ...
    // MUIF_U8G2_U8_MIN_MAX_STEP("B0", &brightness, 0, 20, 1, MUI_MMS_4X_BAR, mui_u8g2_u8_bar_wm_mse_pi),
    MUIF_U8G2_U8_MIN_MAX_STEP("B0", &bright_val, 0, 20, 1, MUI_MMS_4X_BAR, mui_brightness_bar),
...
MUI_STYLE(0)
MUI_XY("B0", 20, 35)
MUI_XYAT("G0", 30, 59, 30, " Back ")
MUI_XYAT("G1", 110, 59, 30, " Ok ")
;
